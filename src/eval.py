...
def evaluate_simulation_results(metrics, tickers, markowitz_paths=None):
    """
    Centralized function to print stats and plot graphs for the simulation output.
    Now includes an optional side-by-side comparison with Markowitz baselines.
    """
    import numpy as np
    import matplotlib.pyplot as plt

    # 1. State-Space Model Metrics
    ...
    # 2. Markowitz Baseline Metrics (Aggregated)
    if markowitz_paths:
        mark_metrics = [risk_metrics(path) for path in markowitz_paths]

        avg_mark_dd = np.mean([m[0] for m in mark_metrics])
        avg_mark_var = np.mean([m[1] for m in mark_metrics])
        avg_mark_ret = np.mean([m[2] for m in mark_metrics])

        print("\n=== MARKOWITZ BASELINE (Avg of Random Portfolios) ===")
        print(f"Markowitz    -> Max DD: {avg_mark_dd:.4f} | Var: {avg_mark_var:.6f} | Avg Return: {avg_mark_ret:.6f}")
    print("---------------------------\n")

    # 3. Visual Comparisons
    # Include the median Markowitz path in the reward comparison if available
    paths_to_plot = [metrics['rewards'], metrics['rewards_greedy'], metrics['rewards_even']]
    labels_to_plot = ["risk-averse", "risk-seeking", "uniform"]

    if markowitz_paths:
        median_markowitz = np.median(markowitz_paths, axis=0)
        paths_to_plot.append(median_markowitz)
        labels_to_plot.append("markowitz-median")

    compare_rewards(paths_to_plot, labels_to_plot)
