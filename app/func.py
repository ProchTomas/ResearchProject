...
def func_F_for_H(phi, phi_0, h_new, h_prev):
    ...
    h_tilde_inv = phi * np.linalg.inv(h_prev) + (1 - phi) * np.linalg.inv(h_new)

    sign_t, logdet_t = np.linalg.slogdet(h_tilde_inv)
    sign_p, logdet_p = np.linalg.slogdet(h_prev)
    sign_n, logdet_n = np.linalg.slogdet(h_new)

    if sign_t <= 0 or sign_p <= 0 or sign_n <= 0:
        return np.inf
